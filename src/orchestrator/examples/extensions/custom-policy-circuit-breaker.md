...
class CircuitBreakerPolicy<C extends BaseContext> extends DefaultPolicy<C> {
  ...
  protected override fail(outcome: StepOutcomeFailed, input: PolicyDecisionInput<C>) {
...
```typescript
import { BaseContext, PipelineStep, StepResult, StepRunMeta, ok, failed } from "@motleywildside/ai-sdk";
...
const orchestrator = new PipelineOrchestrator<PaymentContext>({
