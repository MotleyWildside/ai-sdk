  ...
  async run(ctx: StageCtx, _meta: StepRunMeta): Promise<StepResult<StageCtx>> {
    if (ctx.stage !== "classified") {
      ...
      return {
        ...
        outcome: {
          type: "failed",
          error: new Error(`Expected stage 'classified', got '${ctx.stage}'`),
          retryable: false,
        },
      };
    }
...
```typescript
import { PipelineStep, StepResult, StepRunMeta, ok, failed, PipelineOrchestrator } from "@motleywildside/ai-sdk";
...
const orchestrator = new PipelineOrchestrator<TicketCtx>({
