...
- **Outcome**: Describes _what_ happened semantically (e.g., `ok`, `failed`, `redirect`). It should not decide _where_ to go next.
...
class RetryPolicy extends DefaultPolicy<MyContext> {
  ...
  protected override fail(
    ...
    input: PolicyDecisionInput<MyContext>,
  ): Transition {
...
- `onTransition` _(optional)_ — fired after the policy decides, before the transition is applied
- `onError`
