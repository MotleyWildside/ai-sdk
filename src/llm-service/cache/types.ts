...
export interface CacheProvider {
	...
	/**
	 * Check if a key exists without fetching the value. Optional — cheaper than
	 * `get` on remote stores (e.g. Redis `EXISTS`).
	 */
	has?(key: string): Promise<boolean>;

	/**
	 * Batch-fetch multiple keys in a single round trip. Optional — useful for
	 * RAG retrieval patterns and embedding look-ups.
	 */
