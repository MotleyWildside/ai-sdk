...
async function ingestDocument(documentId: string, text: string): Promise<void> {
  ...
  console.log(
    `Ingested "${documentId}": ${chunks.length} chunks, ${vectorStore.length} total stored`,
  );
}
```
...
registry.register({
  ...
  version: 1,
  systemPrompt:
    "You are a helpful assistant. Answer questions using only the provided context. If the context does not contain the answer, say so.",
  userPrompt: "Context:\n{context}\n\nQuestion: {question}",
