...
When you want to call Anthropic's Claude models directly through the official `@anthropic-ai/sdk` rather than through OpenRouter, you implement the text and streaming provider capabilities yourself. This gives you full control over request parameters, streaming behaviour, and error classification — and lets the service's retry, caching, and prompt-registry machinery work with Anthropic exactly as it does with any built-in provider.

## Concepts covered

- Implementing only the capabilities this provider supports
- Reusing `BaseLLMProvider` for model-prefix matching and provider error helpers
- Mapping `LLMProviderRequest.messages` to Anthropic's `MessageParam` shape
	...
	implements LLMTextProvider, LLMStreamingProvider
{
	...
	protected readonly supportedModelPrefixes = ["claude-"];
	...
	private mapError(err: unknown, model: string): LLMTransientError | LLMPermanentError {
		...
		return new LLMTransientError({
			message: "Network error communicating with Anthropic",
			provider: this.name,
			model,
