...
When you run a local `llama.cpp` HTTP server you want LMService's prompt registry, caching, and retry machinery to work with it just as with any cloud provider. Implementing the text and streaming provider capabilities against the `/completion` endpoint connects local models to the full service stack without any SDK dependency — only Node's built-in `fetch`.

## Concepts covered

- Matching a custom model-name namespace (`"local/"` prefix) in `supportsModel`
- Reusing `BaseLLMProvider` for model-prefix matching and HTTP error helpers
...
- Omitting unsupported operations so `LMService` raises capability errors before provider calls
...
import { BaseLLMProvider, LLMTransientError, LLMPermanentError } from "@motleywildside/ai-sdk";
	...
	implements LLMTextProvider, LLMStreamingProvider
{
	...
	protected readonly supportedModelPrefixes = ["local/"];
	...
	async callStream(request: LLMProviderRequest): Promise<LLMProviderStreamResponse> {
		...
		if (!body) {
			throw new LLMPermanentError({
				message: "llama.cpp returned an empty response body",
				provider: this.name,
				model: request.model,
			});
		}
	...
	private mapHttpError(status: number, model: string): LLMTransientError | LLMPermanentError {
		return this.errorFromHttpResponse(status, {}, model);
	}
}
```
