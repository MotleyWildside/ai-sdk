...
describe("GuidlioLMService — Provider selection (single provider)", () => {
	...
	it("PS-01: defaultProvider resolves — used even if supportsModel returns false", async () => {
		...
		const svc = new GuidlioLMService({ providers: [pA], promptRegistry: reg, defaultProvider: "providerA", logger: log });
	...
	it("PS-02: defaultProvider name not found — logger warns; falls back to auto-select", async () => {
		...
		const log = makeMockLogger();
		const svc = new GuidlioLMService({ providers: [pA], promptRegistry: reg, defaultProvider: "does-not-exist", logger: log });
		await svc.callText({ promptId: "p1" });
	...
	it("PS-03: no defaultProvider — model 'gpt-4o', provider supports 'gpt-' prefix", async () => {
		...
		const svc = new GuidlioLMService({ providers: [pA], promptRegistry: reg });
		await svc.callText({ promptId: "p1" });
		expect(pA.call).toHaveBeenCalledOnce();
	});
	...
	it("PS-04: model supported by none + strictProviderSelection:true — throws", async () => {
		...
		const svc = new GuidlioLMService({ providers: [pA], promptRegistry: reg, strictProviderSelection: true });
	...
	it("PS-05: model supported by none + strictProviderSelection:false — warns; uses first provider", async () => {
		...
		const log = makeMockLogger();
		const svc = new GuidlioLMService({ providers: [pA], promptRegistry: reg, strictProviderSelection: false, logger: log });
		await svc.callText({ promptId: "p1" });
		expect(log.warn).toHaveBeenCalled();
		expect(pA.call).toHaveBeenCalledOnce();
	});
